...
import {
    Calendar, MessageSquare, Target, CheckSquare, Home,
    FileText, LogOut, Bell, Menu, X, Folder, DollarSign, User,
    ChevronRight, Search
} from 'lucide-react';
...
import Sessions from '../components/client/Sessions';
...
const ClientDashboard = () => {
    ...
    const navigation = [
        ...
        { name: 'Sessions', href: '/client/sessions', icon: FileText },
        { name: 'Resources', href: '/client/resources', icon: Folder },
                                        ...
                                        <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                                            {notifications.length === 0 ? (
                                                ...
                                                notifications.map((notif) => (
                                                    <div
                                                        key={notif.id}
                                                        className={`px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors border-b border-slate-100 dark:border-slate-700/50 last:border-0 cursor-pointer ${!notif.read ? 'bg-primary-50/30 dark:bg-primary-900/10' : ''
                                                            }`}
                                                        onClick={() => !notif.read && handleMarkAsRead(notif.id)}
                                                    >
                                                        <div className="flex gap-3">
                                                            <div className={`w-2 h-2 mt-2 rounded-full flex-shrink-0 ${!notif.read ? 'bg-primary-500' : 'bg-slate-300 dark:bg-slate-600'}`} />
                                                            <div>
